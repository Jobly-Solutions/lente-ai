...
// Ejecutar: node upload-file-to-datastore.js
// ========================================

require('dotenv').config({ path: '.env.local' });

console.log('📁 UPLOAD FILE TO DATASTORE...\n');
...
async function braviloRequest(endpoint, method = 'GET', data = null, isFormData = false) {
    ...
    if (data && !isFormData) {
        ...
        console.log(`📦 FormData: ${data ? 'Archivo y metadatos' : 'Sin datos'}`);
    }
...
// Función para subir archivo a datastore usando el endpoint de datasources
async function uploadFileToDatastore(datastoreId, filePath, customId = null) {
    console.log(`📤 Subiendo archivo a datastore ${datastoreId}...`);
    ...
    try {
        ...
        // Hacer la request
        const result = await braviloRequest('/datasources', 'POST', form, true);
...
// Función principal
async function main() {
    console.log('🚀 Iniciando carga de archivo a datastore...\n');
    ...
    try {
        ...
        const uploadResult = await uploadFileToDatastore(
